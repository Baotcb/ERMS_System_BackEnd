    ...
    public async Task<GetApplicationsByJobResponse> Handle(GetApplicationsByJobQuery request, CancellationToken cancellationToken)
    {
        ...
        var dbItems = await query
            ...
            .Select(a => new
            {
                ...
                Source = a.Source,
                CandidateName = a.IsExternal ? a.ExternalCandidateName! : a.Candidate!.User.FullName,
                CandidateEmail = a.IsExternal ? a.ExternalCandidateEmail : a.Candidate!.User.Email,
                CandidatePhone = a.IsExternal ? a.ExternalCandidatePhone : a.Candidate!.User.PhoneNumber,
                ResumeUrl = a.IsExternal ? a.ExternalResumeUrl : (a.Resume != null ? a.Resume.FileUrl : null),
                Stage = a.Stage,
                Status = a.Status,
